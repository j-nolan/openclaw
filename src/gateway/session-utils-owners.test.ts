...
it("canonicalizes merged profile membership before filtering and does not upgrade channel creators", () => {
  ...
  expect(involving.sessions.map((row) => row.key)).toEqual(["agent:main:historical"]);
  ...
  expect(involving.people).toEqual([
    { identity: { type: "profile", id: "current" }, label: "Current", sessionCount: 1 },
  ]);
  const channel = listSessionsFromStore(query).sessions.find((row) => row.key.endsWith(":channel"));
  expect(channel?.createdActor).toEqual({
    type: "human",
    id: "current",
